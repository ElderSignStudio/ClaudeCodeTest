...
<div class="tree-child">
	<a
		href="/users/{user.id}"
		class="group flex items-start gap-2.5 rounded-md px-2 py-1.5 -mx-2 hover:bg-white/5 transition-colors"
		title="View {user.name}'s scout profile"
	>
		<span class="shrink-0 w-7 h-7 rounded-full border border-white/15 group-hover:border-white/25 overflow-hidden bg-white/5 transition-colors">
		...
		<div class="min-w-0 flex-1 flex items-baseline gap-2 flex-wrap">
			...
			{#if typeof user.score === 'number'}
				<span class="text-[11px] tabular-nums text-base-content/50 group-hover:text-base-content/68 transition-colors">+{user.score}</span>
			{/if}
			{#if user.character}
				<p class="text-[11.5px] text-base-content/55 group-hover:text-base-content/68 italic leading-snug truncate transition-colors">{user.character}</p>
			{/if}
		</div>
	</a>
